...
namespace ImageFilters
{
    ...
    public class AdaptiveMedianFilter
    {
        ...
        public AdaptiveMedianFilter(byte[,] image, int WinMxSiz, PictureBox pictureBox, SortingType sortingType)
        {
        ...
        public void Filter()
        {
            ...
            for (int X = 0; X < imageMatrix.GetLength(0); X++)
            {
                ...
                {
                    SrchTruMedian(window(x_init + X, y_init + Y, initImage, WinSize, WinMaxSiz), WinSize, initImage, x_init + X, y_init + Y);

                    imageMatrix[X, Y] = NewPixelVal;
                }

            }
            DisplayImage(imageMatrix, pictureBox);
        ...
        private void SrchTruMedian(byte[] ImgWindow, int Wsize, byte[,] image, int X, int Y)
        {
            /*   if (sortingType == SortingType.COUNTING_SORT)
               { }
               else if (sortingType == SortingType.QUICK_SORT)
               { }
               else if (sortingType == SortingType.BUILT_IN_SORT) { Array.Sort(ImgWindow); }
              */
            ...
            else
            {
                ...
                if (Wsize <= WinMaxSiz)
                {
                    SrchTruMedian(window(X, Y, image, Wsize, WinMaxSiz), Wsize, image, X, Y);
                }
                else
                {
                    NewPixelVal = (byte)Zmed;
